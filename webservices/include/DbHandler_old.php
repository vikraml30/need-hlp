...
class DbHandler {
     ...
     */
    public function createUser($name, $password, $mobile, $otp) {
        $response = array();
        ...
        if (!$this->isUserExists($mobile)) {
            ...
            // insert query
            $stmt = $this->conn->prepare("INSERT INTO users(name, password, mobile, apikey, status) values(?, ?, ?, ?, 0)");
            $stmt->bind_param("ssss", $name, md5($password), $mobile, $api_key);
